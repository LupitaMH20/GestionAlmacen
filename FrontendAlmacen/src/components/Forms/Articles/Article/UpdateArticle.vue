...
const handleSave = async () => {
    try {
        ...
        console.log('id', props.id_mainarticle);
...
<template>
    <Dialog2 title="Actualizar producto" titleButton="Actualizar" :iconP="NotebookPen" :iconT="NotebookPen"
        recordof="Producto" :IconOf="PackageOpen" description="Descripción" :IconD="FileSpreadsheet"
        @cancel="handleCancel" @save="handleSave" v-model:open="isDialogOpen">
        <template #form1>
            <FormArticleU1 v-model:props="data" />
        </template>
        <template #form2>
            <FormArticleU2 v-model:props="data" :company="companies" :category="category" />
        </template>
    </Dialog2>
</template>